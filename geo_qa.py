...
def InsertCountryEntity(graph, doc, countryName, query, relation):
    ...
    for resultUrl in queryResults:
        ...
        if relation in ONTOLOGY_RELATION_PERSON_LST:
            ...
            InsertPersonEntity(graph, doc, resultName, XPATH_QUERY_PERSON_TO_COUNTRY_OF_BIRTH_A,
                               ONTOLOGY_RELATION_BORN_IN)
...
def addOntologyEntity(graph, countryUrl):
    ...
    # Prime minister
    InsertCountryEntity(graph, doc, countryName, XPATH_QUERY_COUNTRY_TO_PRIME_MINISTER,
                        ONTOLOGY_RELATION_PRIME_MINISTER_OF)

    # Population 
    if (countryName in ("Belarus", "Malta", "Dominican_Republic")):
        InsertCountryEntity(graph, doc, countryName, XPATH_QUERY_COUNTRY_TO_POPULATION_UNIQUE1,
                            ONTOLOGY_RELATION_POPULATION_OF)
    elif (countryName == "Russia"):
        InsertCountryEntity(graph, doc, countryName, XPATH_QUERY_COUNTRY_TO_POPULATION_UNIQUE2,
                            ONTOLOGY_RELATION_POPULATION_OF)
    else:
    ...
    if (countryName == "Eswatini"):
        InsertCountryEntity(graph, doc, countryName, XPATH_QUERY_COUNTRY_TO_CAPITAL_ESWATINI,
                            ONTOLOGY_RELATION_CAPITAL_OF)
    else:
...
def getCountriesUrl():
    ...
    doc = lxml.html.fromstring(result.content)
    countryRelUrl = doc.xpath(
        f"{XPATH_QUERY_COUNTRY_URL} | {XPATH_QUERY_COUNTRY_URL_AFGANISTAN} | {XPATH_QUERY_COUNTRY_URL_WESTERN_SAHARA} | {XPATH_QUERY_COUNTRY_URL_CHANNEL_ISLAND}")
